...
import numpy.f2py, numpy.distutils.command.build_src
from distutils.command.sdist import sdist # setuptools userwarning about build_src
...
def write_docs( inp, outf ):
    ...
    with open(outf, "w") as docf:
        docf.write('\n"""Autogenerated from setup.py. Edit in %s please"""\n'%(inp))
        keys = list(docs.keys())
        keys.sort()
        for fname in keys:
            docf.write( '%s = """%s"""\n'%(fname, docs[fname]))
...
### INTERFACE FILENAME : 
PYF = "./src/_cImageD11.pyf"

# Only one built version. Too hard to make this cross plaform. 
# CFLAG# can be used to add extra options per venv
...
###############################################################
# patch build_src to get it to build the pyf file ... (nasty, sorry)
ndcbbr = numpy.distutils.command.build_src.build_src.run
def makepyf_run(self, *args, **kwds):
    log.info("Intercepting numpy.distutils.command.build_src.build_src.run")
    global PYF, csources
    import src.make_pyf
    src.make_pyf.main(PYF, csources)
    log.info("Created %s"%(PYF))
    return ndcbbr(self, *args, **kwds)
numpy.distutils.command.build_src.build_src.run = makepyf_run

# patch f2py to get it to write doc file ... (nasty, sorry)
f2py_run_main = numpy.f2py.run_main
def makedoc_run( *args, **kwds ):
    log.info("Intercepting f2py main to build DOCs for ImageD11")
    DOCF = os.path.join("ImageD11src", "cImageD11_docstrings.py")
    write_docs( PYF, DOCF )
    log.info("Wrote %s from %s working %s"%(PYF,DOCF,os.getcwd()))
    return f2py_run_main(*args, **kwds)
numpy.f2py.run_main = makedoc_run
###############################################################


ekwds = { 'sources' : [ PYF, ] + csources,
