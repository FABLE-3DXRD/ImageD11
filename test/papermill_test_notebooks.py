...
from ImageD11.nbGui.nb_utils import (
    find_datasets_to_process,
    prepare_notebooks_for_datasets,
    notebook_exec_pmill,
)
...
def notebook_route(
    base_dir,
    notebook_paths,
    notebook_param_dicts,
    notebook_out_dir=None,
    skip_dir_check=True,
):
    """
    ...
    for notebook_in_path, notebook_param_dict in zip(
        notebook_paths, notebook_param_dicts
    ):
        notebook_out_path = os.path.join(
            notebook_out_dir,
...
def test_tomographic_route(aroot):
    ...
    notebook_route(
        tomo_dir,
        [os.path.join(scan_nb_prefix, "import_test_data.ipynb")],
        [
            {
                "download_dir": tomo_dir,
...
def test_pbp_route(aroot):
    ...
    nb_params = [
        ...
        ("pbp_2_visualise.ipynb", {"phase_str": "Si", "min_unique": 20}),
        (
...
def test_FeAu_JADB_tomo_y0_struc(aroot):
    # where is the data?
    dataroot = "/data/id11/inhouse2/test_data_3DXRD/S3DXRD/FeAu/RAW_DATA"
    analysisroot = analysis_folder(aroot, "tomo_route")
    # find layers to process
    ...
    nb_params = [
        ...
        (
            "tomo_2_map.ipynb",
            {
                ...
                "manual_threshold": None,
                "hkltol": 0.25,
                # "y0_manual": -15.8,  # manually supply - we don't want the per-layer fit to yield different padding, because then we can't stack
                "update_y0": False,
                "correct_sinos_with_ring_current": False,
                "twin_assign": True,
                "struc_factor_correction": True,
                "cif_for_sf_path": "/data/id11/inhouse2/test_data_3DXRD/S3DXRD/FeAu/pars/Fe.cif",
                "first_tmap_cutoff_level": 0.4,
                "niter": 500,
                "second_tmap_cutoff_level": 0.05,
            },
        ...
        (
            "tomo_2_map_minor_phase.ipynb",
            {
                ...
                "twin_assign": False,
                "struc_factor_correction": True,
                "cif_for_sf_path": "/data/id11/inhouse2/test_data_3DXRD/S3DXRD/FeAu/pars/Au.cif",
                "first_tmap_cutoff_level": 0.01,
                "niter": 500,
                "second_tmap_cutoff_level": 0.01,
                "grain_too_many_px": 10,
            },
        ),
        (
...
def test_FeAu_JADB_pbp_y0_struc(aroot):
    # where is the data?
    dataroot = "/data/id11/inhouse2/test_data_3DXRD/S3DXRD/FeAu/RAW_DATA"
    analysisroot = analysis_folder(aroot, "pbp_route")
    # find layers to process
    ...
    nb_params = [
        ...
        (
            "pbp_1_indexing.ipynb",
            {
                "par_file": "/data/id11/inhouse2/test_data_3DXRD/S3DXRD/FeAu/pars/pars.json",
                "phase_str": "Fe",
                # "y0_manual": -15.8,  # manually specify because it's far from the ideal value
                "minpkint": 5,
                "hkl_tol": 0.03,
                "fpks": 30,
                "ds_tol": 0.008,
        ...
        (
            "pbp_1_indexing.ipynb",
            {
                "par_file": "/data/id11/inhouse2/test_data_3DXRD/S3DXRD/FeAu/pars/pars.json",
                "phase_str": "Au",
                "y0_manual": None,  # should take value from previous notebook
                "minpkint": 5,
                "hkl_tol": 0.03,
                "fpks": 30,
                "ds_tol": 0.008,
...
def test_FeAu_JADB_tomo_fast(aroot):
    # where is the data?
    dataroot = "/data/id11/inhouse2/test_data_3DXRD/S3DXRD/FeAu/RAW_DATA"
    analysisroot = analysis_folder(aroot, "tomo_route")
    # find layers to process
    ...
    nb_params = [
        ...
        (
            "tomo_2_map_minor_phase.ipynb",
            {
                ...
                "grain_too_many_px": 10,
                "run_astra": True,
                "run_astra_on_cluster": True,
                "astra_method": "EM_CUDA"
            },
        ),
        (
            "tomo_3_refinement.ipynb",
...
def test_FeAu_JADB_pbp_fast(aroot):
    # where is the data?
    dataroot = "/data/id11/inhouse2/test_data_3DXRD/S3DXRD/FeAu/RAW_DATA"
    analysisroot = analysis_folder(aroot, "pbp_route")
    # find layers to process
...
if __name__=="__main__":
    ...
    # y0 saving
    # test_FeAu_JADB_tomo_y0_struc(destination_folder)
    # test_FeAu_JADB_pbp_y0_struc(destination_folder)
