...
from xfab.tools import ubi_to_u, ubi_to_rod
...
class indexer:
    ...
    def getind(self, UBI, tol=None, drlv2tmp=None, labelstmp=None):
        ...
        try:
            _ = cImageD11.score_and_assign(UBI, self.gv, tol, drlv2, labels, 1)
        except:
    ...
    def readgvfile(self, filename, quiet=False):
        ...
        if not quiet:
            print(
                "\nGot %d sets of omega values from gv file:"
                % (len(self.omega_ranges) / 2)
            )
        for i in range(len(self.omega_ranges) // 2):  # integer division intended here
            self.omega_fullrange += (
                self.omega_ranges[2 * i + 1] - self.omega_ranges[2 * i]
            )
            if not quiet:
                print(
                    "- Range %1d from %4d to %4d ==> %3d degrees"
                    % (
                        i + 1,
                        self.omega_ranges[2 * i],
                        self.omega_ranges[2 * i + 1],
                        self.omega_ranges[2 * i + 1] - self.omega_ranges[2 * i],
                    )
                )
