...
#      print "Mean drlv old",sum(sqrt(drlv2_old))/drlv2_old.shape[0]
    ...
    def readgvfile(self,filename, quiet=False):
        f=open(filename,"r")
        self.gvname = filename.split('/')[-1]
        self.gvdir = '/'.join(filename.split('/')[:-1])
        self.gvstem = self.gvname.split('.')[0]
...
#         print  "...."
    ...
    def gspotter_save_ini(self, filename):
        f = open(filename, 'w')
        try:
            f.write(self.gsini)
            f.close()
            self.gsinifile = filename
        except:
            raise

    def gspotter_initialise_ini(self):
        self.gscards=[]
        self.gsini =''

    def gspotter_add_card(self,card):
        """
        ...
        if len(card) == 2:
            try:
                self.gscards.append(card)
            except AttributeError:
                self.gscards = [card]

    def gspotter_make_ini(self):
        self.gsini = '! File generated by ImageD11\n'
        inicomments = {}
        inicomments['spacegroup'] = '! spacegroup [space group nr]'
        inicomments['dsrange'] = '! dsrange [min max], d-spacing range, multiple ranges can be specified'
        inicomments['tthrange'] = '! tthrange [min max], multiple ranges can be specified'
        inicomments['etarange'] = '! etarange [min max], multiple ranges can be specified'
        inicomments['domega'] = '! domega [stepsize] in omega, degrees'
        inicomments['omegarange'] = '! omegarange [min max] degrees, multiple ranges can be specified'
        inicomments['filespecs'] = '! filespecs [gvecsfile grainsfile]'
        inicomments['cuts'] = '! cuts [min_measuments min_completeness min_uniqueness]'
        inicomments['uncertainties'] = '! uncertainties [sigma_tth sigma_eta sigma_omega] in degrees'
        inicomments['nsigmas'] = '! nsigmas [Nsig] : maximal deviation in sigmas'
        inicomments['minfracg'] = '! stop search when minfracg (0..1) of the gvectors have been assigned to grains'
        inicomments['Nhkls_in_indexing'] = '! Nhkls_in_indexing [Nfamilies] : use first Nfamilies in indexing'
        inicomments['random'] = '! use randomly chosen orientation seeds #trials'
        inicomments['positionfit'] = '! fit the position of the grain'

        for k, v in self.gscards:
            self.gsini += k + ' ' + v
            if k in inicomments:
                self.gsini += '        ' + inicomments[k]
            self.gsini += '\n'

    def gspotter_run(self):
        import os
        ret = os.system('GrainSpotter.0.90 %s' %(self.gsinifile))
        return ret