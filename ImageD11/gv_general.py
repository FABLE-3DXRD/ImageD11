...
class rotation_axis:
    ...
    def rotate_vectors(self, vectors, angles=None):
        ...
        rp += apa * (1 - np.cos(q))
        rp += np.sin(q) * np.transpose(np.cross(self.direction, np.transpose(p)))
        return rp
    ...
    def to_matrix(self):
        ...
        w = np.transpose(np.array([[0, -dz, dy], [dz, 0, -dx], [-dy, dx, 0]], float))
        st = math.sin(math.radians(self.angle))
        ct = math.cos(math.radians(self.angle))
        self.matrix = (
            np.identity(3, float) * ct - st * w + (1 - ct) * e * np.transpose(e)
        )
...
def axis_from_matrix(m):
    ...
    if arg == 1:
        ...
        direc = np.array(
            [m[2, 1] - m[1, 2], m[0, 2] - m[2, 0], m[1, 0] - m[0, 1]], float
        )
...
def k_to_g(
    k,  # scattering vectors in the laboratory
...
def wedgechi(wedge=0.0, chi=0.0):
    """
    in transform:
    g = omega . chi . wedge .k
    """
...
def chiwedge(chi=0.0, wedge=0.0):
    """
    in transform:
    g = omega . chi . wedge .k
    """
