...
PARAMETERS = [
    ...
    par(
        ...
        can_vary=True,
        stepsize=0.1,
    ),  # this could actually vary - a bit crazy?
    par(
        "y_size",
        46.77648,
...
class transformer:
    ...
    def loadfiltered(self, filename):
        ...
        if (
            ("sc" in self.colfile.titles)
            and ("fc" in self.colfile.titles)
            and ("omega" in self.colfile.titles)
        ):
        ...
        if "spot3d_id" not in self.colfile.titles:
            self.colfile.addcolumn(list(range(self.colfile.nrows)), "spot3d_id")
    ...
    def setxyomcols(self, xname, yname, omeganame):
        ...
        logging.warning(
            "titles are %s  %s  %s" % (self.xname, self.yname, self.omeganame)
        )
    ...
    def compute_histo(self, colname):
        ...
        if weight:
            bins, hist, hpk = transform.compute_tth_histo(
                self.getcolumn(colname),
                weight=True,
                weights=self.getcolumn("sum_intensity"),
                **self.parameterobj.get_parameters()
            )
        else:
            bins, hist, hpk = transform.compute_tth_histo(
                self.getcolumn(colname), **self.parameterobj.get_parameters()
            )
    ...
    def filter_min(self, col, minval):
        ...
        logging.info("Number of peaks before filtering = %d" % (self.colfile.nrows))
        self.colfile.filter(mask)
        logging.info("Number of peaks after filtering = %d" % (self.colfile.nrows))
    ...
    def fit(self, tthmin=0, tthmax=180):
        ...
        for i in range(len(self.theoryds)):
            dsc = self.theoryds[i]
            tthcalc = math.asin(dsc * w / 2) * 360.0 / math.pi  # degrees
            if tthcalc > tthmax:
                break
            elif tthcalc < tthmin:
                continue
    ...
    def addcellpeaks(self, limit=None):
        ...
        if limit is None:
            ...
            highest = limit
        w = pars["wavelength"]
        ds = 2 * numpy.sin(transform.radians(highest) / 2.0) / w
        self.dslimit = ds
        ...
        if "cell_sg" in pars:
            ...
            for i in range(len(self.theorypeaks)):
                tths.append(2 * numpy.arcsin(w * self.theorypeaks[i][0] / 2.0))
            ...
            tths = [numpy.arcsin(w * dstar / 2) * 2 for dstar in self.unitcell.ringds]
    ...
    def write_graindex_gv(self, filename):
        ...
        if "gx" not in self.colfile.titles:
            self.computegv()
        gv = [self.getcolumn("gx"), self.getcolumn("gy"), self.getcolumn("gz")]
        ...
        if "sum_intensity" in self.colfile.titles:
            ...
            ints = self.getcolumn("sum_intensity") * self.getcolumn("Number_of_pixels")
        elif ("avg_intensity" in self.colfile.titles) and (
            "npixels" in self.colfile.titles
        ):
            ints = self.getcolumn("sum_intensity") * self.getcolumn("npixels")
        else:
    ...
    def write_pyFAI(self, filename, tthmin=0, tthmax=180):
        """
        Write file for Jerome Kieffer's pyFAI fitting routine to use and run
        the refinment...
        """
        try:
            import pyFAI
