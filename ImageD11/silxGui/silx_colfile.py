...
class silxqtcolfile(object):
    ...
    def savecolfile(self):
        ...
        try:
            ...
            _ = silx.gui.qt.QMessageBox.about(
                self.scatter_widget, "Fail", "Saving failed - bad filename?"
            )

    def loadColfile(self, filename=None):
        """Read in a new file"""
        if filename is None or filename is False:
            filename = silx.gui.qt.QFileDialog(
                self.win, "Columnfile"
            ).getOpenFileName()[0]
        try:
    ...
    def loadparameters(self):
        ...
        if not os.path.exists(fname):
            _ = silx.gui.qt.QMessageBox.about(
                self.scatter_widget, "Fail", "Setting parameters failed - bad filename?"
            )
