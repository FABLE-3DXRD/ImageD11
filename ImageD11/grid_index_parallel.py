...
from ImageD11 import indexing, transformer, cImageD11
...
def domap(pars, colfile, grains, gridpars):
    ...
    for tol in gridpars["TOLSEQ"]:
        ...
        if "NUNIQ" in gridpars:
            ...
            def keep(g):
                return g.nuniq > gridpars["NUNIQ"] and g.npks > gridpars["NPKS"]

        else:

            def keep(g):
                return g.npks > gridpars["NPKS"]
...
def doindex(gve, x, y, z, w, gridpars):
    ...
    grains = [
        grain.grain(ubi, [x, y, z])
        for ubi in myindexer.ubis
...
def test_many_points(args):
    ...
    for i, (t_x, t_y, t_z) in enumerate(translations):
        ...
        gve = np.vstack(
            (
                mytransformer.colfile.gx,
                mytransformer.colfile.gy,
        ...
        if ng > 0:
            grains = domap(
                mytransformer.parameterobj, mytransformer.colfile, grains, gridpars
            )
            nk = len(grains)
            if len(grains) > 0:
            ...
            nk = 0
        sys.stderr.write(
            "         % 6.2f%% Position %d %d %d" % (i / ni, t_x, t_y, t_z)
    ...
    def add(self, grains):
        for i, gnew in enumerate(grains):
            ...
            for gold in self.uniqgrains:
                ...
                arg = (aumis[:, 0, 0] + aumis[:, 1, 1] + aumis[:, 2, 2] - 1.0) / 2.0
...
def grid_index_parallel(fltfile, parfile, tmp, gridpars, translations):
    ...
    ul = uniq_grain_list(
        gridpars["SYMMETRY"], gridpars["toldist"], gridpars["tolangle"]
    )
    lastsave = 0
