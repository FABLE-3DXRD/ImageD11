...
def props__(scan, i, algorithm="lmlabel", wtmax=None):
    """
...
def props(scan, i, algorithm="lmlabel", wtmax=None):
    ...
    Labels the peaks with lmlabel
    Assumes a regular scan for labelling frames
    returns ( row, properties[(s1,sI,sRow,sCol,frame,sRow2,sCol2,sRowCol),:], pairs, scan )

    Row layout:
        r[0]  number of pixels
        r[1]  sum of (clipped) intensity          Σ I
        r[2]  weighted row sum                    Σ I·s
        r[3]  weighted col sum                    Σ I·f
        r[4]  frame index
        r[5]  weighted row² sum                   Σ I·s²
        r[6]  weighted col² sum                   Σ I·f²
        r[7]  weighted row·col sum                Σ I·s·f

    Centroids and second moments are left un-normalised so downstream
    code can accumulate across frames before dividing by Σ I.
    König's theorem then gives:
        σ²_ss = r[5]/r[1] - (r[2]/r[1])²
        σ²_ff = r[6]/r[1] - (r[3]/r[1])²
        σ²_sf = r[7]/r[1] - (r[2]/r[1])·(r[3]/r[1])
    """
    scan.sinorow = i
    getattr(scan, algorithm)(countall=False)
    npks = scan.total_labels
    r = np.empty((10, npks), np.int64)
    s = 0
    j0 = i * scan.shape[0]
    for j in range(scan.shape[0]):
        if scan.nnz[j] == 0:
        ...
        f0 = scan.getframe(j)
        e = s + scan.nlabels[j]
        lbl = f0.pixels["labels"]
        row = f0.row
        col = f0.col

        r[0, s:e] = np.bincount(lbl)[1:]
        ...
        if wtmax is not None:
            ...
            if m.any():
                wt[m] = wtmax
        signal = np.bincount(lbl, weights=wt)[1:]
        if signal.min() < 1:
            print(
                "Bad data",
                scan.hname,
            ...
            raise Exception("bad data")
        r[1, s:e] = signal
        r[2, s:e] = np.bincount(lbl, weights=row * wt)[1:]
        r[3, s:e] = np.bincount(lbl, weights=col * wt)[1:]
        r[4, s:e] = j + j0
        # shift by the per-frame bounding box origin before squaring
        # any constant per-frame offset cancels out in the variance
        row0 = row.min()
        col0 = col.min()
        ds   = row - row0
        dc   = col - col0
        r[5, s:e] = np.bincount(lbl, weights=ds * ds * wt)[1:]
        r[6, s:e] = np.bincount(lbl, weights=dc * dc * wt)[1:]
        r[7, s:e] = np.bincount(lbl, weights=ds * dc * wt)[1:]
        r[8, s:e] = row0     # same scalar broadcast to all peaks in this frame
        r[9, s:e] = col0
        s = e
...
class pks_table:
    ...
    def create(self, npk):
        ...
        self.pk_props = self.share("pk_props", shape=(10, s[0]), dtype=np.int64)
    ...
    def save(self, h5name, group="pks2d", rc=False):
        ...
        with h5py.File(h5name, "a") as hout:
            ...
            ds[:] = self.pk_props
            ds.attrs["description"] = "[ ( s1, sI, srI, scI, id, srrI, sccI, srcI, s0, f0 ),  Npks ]"
            ds = grp.require_dataset(
                name="npk",
    ...
    def pk2d__(self, omega, dty, scale_factor=None):
        """
        scale_factor: provide scale_factor with same shape as omega/dty
        """
    ...
    def pk2d(self, omega, dty, scale_factor=None):
        ...
        """
        s1, sI, srI, scI, frm     = self.pk_props[:5]
        srrI, sccI, srcI, s0, f0  = self.pk_props[5:]

        s_raw, f_raw, omegapk, dtypk, fwhm_major, fwhm_minor, anisotropy = \
            n_pk2d(s1, sI, srI, scI, frm, omega, dty, srrI, sccI, srcI, s0, f0)
...
def n_pk2d__( s1, sI, srI, scI, frm, omega, dty ):
    n = len(s1)
...
@numba.njit(parallel=True)
def n_pk2d(s1, sI, srI, scI, frm, omega, dty, srrI, sccI, srcI, s0, f0):
    n = len(s1)
    s_raw      = np.empty(n, np.dtype('d'))
    f_raw      = np.empty(n, np.dtype('d'))
    omegapk    = np.empty(n, np.dtype('d'))
    dtypk      = np.empty(n, np.dtype('d'))
    fwhm_major = np.empty(n, np.dtype('d'))
    fwhm_minor = np.empty(n, np.dtype('d'))
    anisotropy = np.empty(n, np.dtype('d'))
    ...
    for i in numba.prange(n):
        ...
        ds_bar = s_bar - s0[i]    
        df_bar = f_bar - f0[i]
        var_ss = max(srrI[i] / sI[i] - ds_bar * ds_bar, 0.0)
        var_ff = max(sccI[i] / sI[i] - df_bar * df_bar, 0.0)
        var_sf =     srcI[i] / sI[i] - ds_bar * df_bar

        # Eigenvalues of the 2×2 covariance matrix
        m   = 0.5 * (var_ss + var_ff)
        d   = 0.5 * (var_ss - var_ff)
        rad = np.sqrt(max(d * d + var_sf * var_sf, 0.0))

        lam_major = m + rad
        lam_minor = max(m - rad, 0.0)

        fwhm_major[i] = FWHM_FACTOR * np.sqrt(var_ss)
        fwhm_minor[i] = FWHM_FACTOR * np.sqrt(var_ff)
        anisotropy[i] = lam_major / lam_minor if lam_minor > 0.0 else np.nan

    return s_raw, f_raw, omegapk, dtypk, fwhm_major, fwhm_minor, anisotropy
