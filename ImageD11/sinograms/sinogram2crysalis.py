...
if not hasattr(fabio.app.eiger2crysalis.Converter, "patched"):
    fabio.app.eiger2crysalis.Converter.common_headers = name_decorator(
        fabio.app.eiger2crysalis.Converter.common_headers
    )
            ...
            with readlock:
                with h5py.File(os.path.join(ds.datapath, sourcefile), "r") as hin:
                    ...
                    for t in todo:
                        ...
                        try:
                            chunks.append(
                                (num, dset.id.read_direct_chunk((t, 0, 0))[1])
                            )
                        except:
...
def makecmdline(
    ...
    return (
        "eiger2crysalis /dev/null -w %f --beam %.2f %.2f --omega=-%d+index*%f --distance=%.3f -o %s/esperanto/frame_%d_{index}.esperanto"
...
def makecmdline(
    ...
    return (
        "eiger2crysalis sum_lpcmoX1_slice1.h5 -w %f --flip-lr --calc-mask False --beam %.2f %.2f --omega=-%d+index*%f --distance=%.3f -o %s/frame_%d_{index}.esperanto"
        % (wvln, xbeam, ybeam, omegastart, step, distance, savepath, run)
    )
