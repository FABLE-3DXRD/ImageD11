...
def read_segment(args):
    ...
    with h5py.File(open(hname, "rb"), "r") as hin:
        ...
        while s < end:
            e = min(end, s + b)
            hin[dset]["pk_props"].read_direct(
                tbl.pk_props, np.s_[:, s:e], np.s_[:, s:e]
            )
            hin[dset]["glabel"].read_direct(tbl.glabel, np.s_[s:e], np.s_[s:e])
            s = e
...
def create(hname, dset):
    with h5py.File(hname, "r") as hin:
        ...
        for name in "pk_props", "glabel":
            dset = hin["pks2d"][name]
            setattr(tbl, name, tbl.share(name, shape=dset.shape, dtype=dset.dtype))
        del dset
...
def readpartial(args):
    ...
    p = multiprocessing.Process(
        target=read2pipe, args=(write_end, hname, dset, selection)
    )
    p.start()
    output = ary[selection]
    output[:] = np.frombuffer(read_end.recv_bytes(), dtype=output.dtype).reshape(
        output.shape
    )
    p.join()
    read_end.close()
