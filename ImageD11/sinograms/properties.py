...
class pks_table:
    ...
    def pk2dmerge(self, omega, dty):
        ...
        _ = numbapkmerge(self.glabel, self.pk_props, omega, dty, out)
        allpks = {
            "s_raw": out[2] / out[1],
            "f_raw": out[3] / out[1],
            "omega": out[4] / out[1],
...
def process(qin, qshm, qout, hname, scans, options):
    ...
    for i in range(start, end + 1):
        ...
        pkid[i] = np.concatenate(
            (
                [
                    0,
                ],
                np.cumsum(scan.nlabels),
            )
        )
...
def goforit(ds, sparsename, options):
    ...
    with mp.Pool(
        NPROC,
        initializer=process,
        initargs=(qin, qshm, qresult, sparsename, ds.scans, options),
    ) as pool:
