...
def refine_vector(v, gv, tol=0.25, ncycles=25, precision=1e-6):
    ...
    for i in range(ncycles):
        ...
        one_over_s = np.where(s / s.max() < 1e-6, 1, 1.0 / s)
...
class grid:
    ...
    def gv_to_grid_new(self, gv):
        ...
        hmx = hkl.max(axis=1) < (self.npx / 2.0 - 2.0)
        hmn = hkl.min(axis=1) > (2.0 - self.npx / 2.0)
        ...
        for cor in [
            ...
            ind = (
                thkl[:, 0] * grid.shape[1] * grid.shape[2]
                + thkl[:, 1] * grid.shape[1]
                + thkl[:, 2]
            )
    ...
    def fft(self):
        ...
        start = time.time()
        self.patty = np.ascontiguousarray(abs(np.fft.fftn(self.grid)), np.float32)
    ...
    def props(self):
        ...
        logging.info("Patterson info " + str(self.patty.shape) + str(type(self.patty)))
    ...
    def read_peaks(self, peaksfile):
        ...
        self.px = colf.omega
        self.px = (
            np.where(self.px > self.npx / 2, self.px - self.npx, self.px) * self.rlgrid
        )
        self.py = colf.sc
        self.py = (
            np.where(self.py > self.npx / 2, self.py - self.npx, self.py) * self.rlgrid
        )
        self.pz = colf.fc
        self.pz = (
            np.where(self.pz > self.npx / 2, self.pz - self.npx, self.pz) * self.rlgrid
        )
        self.UBIALL = np.array([self.px, self.py, self.pz]).T
        logging.info(
            "Number of peaks found %d  %f/s, now fit some"
    ...
    def slow_score(self):
        ...
        mag_v = np.sqrt(self.px * self.px + self.py * self.py + self.pz * self.pz)
        f = open("fft.pks", "w")
        for i in range(nv):
            j = order[i]
            f.write(
                "%d %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %7d "
                % (
                    i,
                    self.px[j],
                    self.py[j],
