...
outif = {"P": P, "A": I, "B": B, "C": C, "I": I, "F": F, "R": R}
...
class unitcell:
    ...
    def __init__(self, lattice_parameters, symmetry="P", verbose=0):
        ...
        if self.lattice_parameters.shape[0] != 6:
            raise Exception(
                "You must supply 6 lattice parameters\n"
                + "      a,b,c,alpha,beta,gamma"
            )
        self.symmetry = symmetry
        if self.symmetry not in ["P", "A", "B", "C", "I", "F", "R"]:
            raise Exception("Your symmetry " + self.symmetry + " was not recognised")
        ...
        try:
            ...
            raise Exception(
                "Unit cell was degenerate, could not determine"
        ...
        if verbose == 1:
            print(
                self.astar, self.bstar, self.cstar, self.alphas, self.betas, self.gammas
            )
        ...
        if verbose == 1:
            print(self.B)
        if verbose == 1:
            print(np.dot(np.transpose(self.B), self.B) - self.gi)  # this should be zero
        self.hkls = None
        self.peaks = None
        self.limit = 0
    ...
    def gethkls_xfab(self, dsmax, spg):
        ...
        """
        stl_max = dsmax / 2.0
        raw_peaks = tools.genhkl_all(
            self.lattice_parameters, 0, stl_max, sgname=spg, output_stl=True
        )
        peaks = []
    ...
    def getanglehkls(self, ring1, ring2):
        ...
        """
        if (
            self.ringtol != self.anglehkl_cache["ringtol"]
            or (self.B != self.anglehkl_cache["B"]).any()
        ):
            self.anglehkl_cache = {
                "ringtol": self.ringtol,
                "B": self.B,
    ...
    def orient(self, ring1, g1, ring2, g2, verbose=0, crange=-1.0):
        """
        ...
        if crange > 0:
            ...
            if i > 0 and (
                i == len(c2ab)
                or (fabs(costheta - c2ab[i - 1]) < fabs(costheta - c2ab[i]))
            ):
                best = [
                    i - 1,
                ]
...
def filter_pairs(h1, h2, c2a, B, BI, tol=1e-5):
    ...
    inds = list(np.arange(1, len(dc) + 1, dtype=int)[dc]) + [
        len(c2as) - 1,
    ]
    p = 0  # previous
    for i in inds:
        ...
        gb = np.dot(B, hb)
        assert (
            abs(np.dot(ga, gb) / np.sqrt(np.dot(ga, ga) * np.dot(gb, gb)) - c2as[p])
            < 2e-8
        ), "mixup in filter_pairs"
        gobs = np.array((ga, gb, (0, 0, 0)), float)
        UBI = gobs.copy()
        cImageD11.quickorient(UBI, BT)
        gtest = [
            np.dot(np.linalg.inv(UBI), HKL0).T.copy(),
        ]
...
def unitcell_from_parameters(pars):
    ...
    cell = unitcell(
        [pars.get("cell_%s" % (s)) for s in parnames],
