...
class solver:
    ...
    def MVStiffness(self):
        return formStiffnessMV(
            crystal_system=self.crystal_symmetry,
            c11=self.c11,
            c12=self.c12,
            c13=self.c13,
            c14=self.c14,
    ...
    def compute_write_eps_sig(self, outputfile):
        ...
        if self.ubis is not None:
            ...
            f.write("##############################################\n")
            f.write(
                "cell__a cell__b cell__c cell_alpha cell_beta cell_gamma u11 u12 u13 u21 u22 u23 u31 u32 u33 "
            )
            f.write(
                "eps11_c eps22_c eps33_c eps12_c eps13_c eps23_c eps11_s eps22_s eps33_s eps12_s eps13_s eps23_s "
            )
            f.write(
                "sig11_c sig22_c sig33_c sig12_c sig13_c sig23_c sig11_s sig22_s sig33_s sig12_s sig13_s sig23_s\n"
            )
            ...
            for ubi in self.ubis:
                ...
                epsM = [
                    [
                        ...
                        eps[1],
                        eps[2],
                    ],  # write the strain tensor list as a matrix
                    [eps[1], eps[3], eps[4]],
                    [eps[2], eps[4], eps[5]],
                ]
                ...
                try:
                    sigM = strain2stress(
                        np.array(epsM), self.MVStiffness()
                    )  # write the stress tensor as a symmetric matrix in crystal co-ordinates
                    sigS = np.dot(U, np.dot(sigM, U.T))  # sigma in sample co-ordinates
                except:
                    print(
                        "couldn't compute stress! please check the crystal_symmetry parameters and elastic constants"
                    )
                    writestress = False
                ...
                for i, j in [(0, 0), (1, 1), (2, 2), (0, 1), (0, 2), (1, 2)]:
                    ligne = ligne + " " + str(100.0 * np.array(epsM)[i, j])
                for i, j in [(0, 0), (1, 1), (2, 2), (0, 1), (0, 2), (1, 2)]:
                    ligne = ligne + " " + str(100.0 * epsS[i, j])
                if writestress is True:
                    for i, j in [(0, 0), (1, 1), (2, 2), (0, 1), (0, 2), (1, 2)]:
