...
class indexer:
    ...
    def updatecolfile(self):
        if "xl" not in self.cf.titles:
            ...
            xl, yl, zl = transform.compute_xyz_lab(pks, **self.transformpars.parameters)
        ...
        tth, eta = transform.compute_tth_eta_from_xyz(
            self.peaks_xyzT.T, om * osign, **self.transformpars.parameters
        )
        self.cf.addcolumn(
            tth,
            "tth",
        ...
        self.cf.addcolumn(np.sqrt(gx * gx + gy * gy + gz * gz), "modg")
    ...
    def tthcalc(self, hkls=None):
        ...
        self.unitcell.ringtth = [
            2 * np.degrees(np.arcsin(ds * wvln / 2)) for ds in self.unitcell.ringds
        ]
    ...
    def printringassign(self):
        ...
        for j in range(len(dsr))[::-1]:
            ...
            print(
                "Ring %-3d (%3d,%3d,%3d)  %3d  %5d  %5d  %5d  %.4f"
                % (
                    j,
                    h[0],
                    h[1],
                    h[2],
                    len(self.unitcell.ringhkls[dsr[j]]),
                    ind.sum(),
                    n_indexed,
                    n_to_index,
    ...
    def search1d(
        ...
        """
        gv = np.array(
            (self.cf.gx[gvec_id], self.cf.gy[gvec_id], self.cf.gz[gvec_id]), float
        )
        print(gvec_id)
        if hkl is None:
        ...
        scores = []
        ubis = []
        # gc = np.dot(ub0, hkl) # gcalc
        angs = np.radians(np.linspace(angstart, angend, nang))
        ubis = [np.dot(ubi0, Rotation.from_rotvec(nobs * a).as_matrix()) for a in angs]
        scores = [cImageD11.score(ubi, allgve, tol) for ubi in ubis]
        return scores, ubis
    ...
    def choose(self, gnum, tol):
        ...
        angend = 360
        nang = 3600
        s, ubis = self.search1d(
            idp, tol=tol, angstart=angstart, angend=angend, nang=nang
        )
    ...
    def refine(self, ubi, translation=(0, 0, 0), inds=None, hkls=None, tol=0.05):
        """
        ...
        res, ier = scipy.optimize.leastsq(
            fun,
            x0,
            args,
        )  #
        ...
        for k, g in enumerate(gl):
            if len(sys.argv[3]) == len("fit"):
                for j, tol in enumerate([0.05, 0.02, 0.01, 0.0075]):
                    ...
                    ubi, t = i.refine(
                        g.ubi, translation=g.translation, inds=inds, hkls=hkls, tol=tol
                    )
                    g.translation = t
                    g.set_ubi(ubi)
            else:
