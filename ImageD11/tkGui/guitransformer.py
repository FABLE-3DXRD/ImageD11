...
class guitransformer:
    ...
    def loadfiltered(self):
        filename = self.parent.opener.show(
            title="File containing filtered peaks",
            filetypes=[("filtered peaks", "*.flt"), ("All Files ", "*")],
        )
        self.parent.guicommander.execute("transformer", "loadfiltered", filename)

    def loadfileparameters(self):
        filename = self.parent.opener.show(
            title="File containing detector parameters",
            filetypes=[
                ("parameter files", "*.par"),
        ...
        self.parent.guicommander.execute("transformer", "loadfileparameters", filename)

    def saveparameters(self, filename=None):
        if filename == None:
            filename = self.parent.saver.show(title="File to save detector parameters")
        self.parent.guicommander.execute("transformer", "saveparameters", filename)
    ...
    def editparameters(self):
        ...
        d = listdialog(
            self.parent, items=pars, title="Detector parameters", logic=logic
        )
        self.parent.guicommander.execute(
            "transformer", "parameterobj.set_parameters", d.result
        )
        # wtf d.fv
        vars = []
        print("d.fv", d.fv)
        ...
        self.parent.guicommander.execute(
            "transformer", "parameterobj.set_varylist", vars
        )
    ...
    def plothisto(self, nbins=None):
        if nbins is None:
            nbins = self.parent.guicommander.execute(
                "transformer", "parameterobj.get", "no_bins"
            )
            doweight = self.parent.guicommander.execute(
                "transformer", "parameterobj.get", "weight_hist_intensities"
            )
            d = listdialog(
                self.parent,
            ...
            self.parent.guicommander.execute(
                "transformer", "parameterobj.set", "no_bins", nbins
            )

        bins, hist = self.parent.guicommander.execute(
            "transformer", "compute_tth_histo"
        )
        self.parent.twodplotter.adddata(
            (
                "2Theta/Eta",
    ...
    def savehisto(self, nbins=None):
        ...
        self.parent.guicommander.execute(
            "transformer", "save_tth_his", filename, bins, hist
        )
    ...
    def filterhisto(self):
        ...
        min_bin_prob = self.parent.guicommander.execute(
            "transformer", "parameterobj.get", "min_bin_prob"
        )
        ...
        d = listdialog(
            ...
            items={
                ...
                "weight_hist_intensities": doweight,
            },
        ...
        self.parent.guicommander.execute(
            "transformer", "filter_min", "tth_hist_prob", min_bin_prob
        )
    ...
    def savecolfile(self):
        ...
        self.parent.guicommander.execute("transformer", "write_colfile", filename)
    ...
    def write_pyFAI(self):
        ...
        self.parent.guicommander.execute(
            "transformer", "write_pyFAI", filename, tthmin, tthmax
        )
