...
class myOpengl(Opengl):
    ...
    def tkRedraw(self, *dummy):
        ...
        GL.glClearColor(self.r_back, self.g_back, self.b_back, 0.0)
...
class plot3d(Tk.Toplevel):
    ...
    def readimage(self, image):
        ...
        d = numpy.reshape(
            numpy.clip(self.imageobj.data, mi, mx), shape
        )  # makes a clipped copy
        d = 255.0 * (d - mi) / (mx - mi)  # scale intensity
        ...
        gve = transform.compute_g_vectors(
            tth, eta, omega * self.pars["omegasign"], self.pars["wavelength"]
        )
        self.pts = []
        print("Setting up image mapping", p.shape, gve.shape)
        for i in range(pk.shape[1]):
            self.pts.append(
                [pk[1, i] / 1024.0, pk[0, i] / 1024.0, gve[0, i], gve[1, i], gve[2, i]]
            )
        # for p in self.pts:
        #    print p
        self.setupTexture()
    ...
    def scorecolor(self, i=0):
        ...
        if self.ubis is not None:
            ...
            for u, i in zip(self.ubis, list(range(len(self.ubis)))):
                ...
                ind = numpy.compress(
                    numpy.less(scores, 0.05 * 0.05), numpy.arange(self.xyz.shape[0])
                )
    ...
    def changedata(self, xyz=None):
        ...
        GL.glColorPointer(
            3, GL.GL_FLOAT, 0, self.color.astype(numpy.float32).tostring()
        )
    ...
    def redraw(self, o):
        ...
        if self.ubis is not None and len(self.ubis) == 1:
            hkl = numpy.dot(numpy.linalg.inv(self.ubis[0]), numpy.identity(3, float)).T
            # print hkl
        else:
        ...
        if self.tex:
            ...
            GL.glEnable(GL.GL_TEXTURE_2D)
            GL.glColor4f(0.0, 1.0, 0.0, 1.0)  # red
