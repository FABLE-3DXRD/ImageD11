...
class twodplot(Tk.Frame):
    def __init__(self, parent=None, data=None, quiet="No"):
        ...
        Tk.Button(master=self.bf1, text="Clear", command=self.clear).pack(
            self.pack_opts
        )
        Tk.Button(master=self.bf1, text="Save Plot", command=self.printplot).pack(
            self.pack_opts
        )
        ...
        Tk.Button(
            master=self.bf1, text=">", command=lambda: self.keypress(self.a.panx, 1)
        ).pack(self.pack_opts)
        Tk.Button(
            master=self.bf1, text="<", command=lambda: self.keypress(self.a.panx, -1)
        ).pack(self.pack_opts)
        Tk.Button(
            master=self.bf1, text="^", command=lambda: self.keypress(self.a.pany, -1)
        ).pack(self.pack_opts)
        Tk.Button(
            master=self.bf1, text="v", command=lambda: self.keypress(self.a.pany, 1)
        ).pack(self.pack_opts)
        ...
        Tk.Button(
            master=self.bf2,
            text="UnZoomX",
            command=lambda: self.keypress(self.a.zoomx, -1),
        ).pack(self.pack_opts)
        Tk.Button(
            master=self.bf2,
            text="ZoomX",
        ...
        Tk.Button(
            master=self.bf2,
            text="ZoomY",
            command=lambda: self.keypress(self.a.zoomy, 1),
        ).pack(self.pack_opts)
        Tk.Button(
            master=self.bf2,
            text="UnZoomY",
        ...
        Tk.Button(
            master=self.bf2,
            text="Autoscale",
            command=lambda: self.keypress(self.autoscale),
        ).pack(self.pack_opts)
        Tk.Button(
            master=self.bf2,
            text="Autoscale Y",
            command=lambda: self.keypress(self.autoscaley, None),
        ).pack(self.pack_opts)
    ...
    def printplot(self):
        fn = filedialog.asksaveasfilename(
            title="File name to print to", defaultextension="png"
        )
        print(fn)
        ...
        if e.lower() in [".png", ".ps", ".eps", ".bmp", ".raw", ".rgb"]:
            ...
            messagebox.showinfo(
                "Sorry", "I can only make output in these formats" + str(extns)
            )
                ...
                if "plottype" in item.d:
                    ...
                    if self.quiet == "No":
                        if messagebox.askyesno(
                            ...
                            self.a.plot(
                                item.x[: self.maxpoints], item.y[: self.maxpoints], **po
                            )
                        else:
    ...
    def on_move(self, event):
        ...
        if (self.startx != event.x) and (self.starty != event.y):
            if self.rubberbandbox is not None:
                self.tkc.delete(self.rubberbandbox)
            self.rubberbandbox = self.tkc.create_rectangle(
                self.startx, self.starty, x, y, outline="green"
            )
...
if __name__ == "__main__":
    ...
    dat = data(
        x, np.sin(2 * pi * x) + 5, np.sqrt(np.sin(2 * pi * x) + 5), {"title": "sin x"}
    )
