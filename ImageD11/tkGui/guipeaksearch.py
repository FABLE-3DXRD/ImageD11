...
class guipeaksearcher:
    ...
    def searchraw(self):
        ...
        showinfo(
            "Sorry",
            """Not implemented for gui, please use the jonpeaksearch script on amber/crunch for now""",
        )
    ...
    def harvestpeaks(self):
        ...
        if self.quiet == "No":
            ans = askyesno(
                "Have you selected a sensible range of images?",
                """
   ...
   """,
        ...
        self.parent.guicommander.execute(
            "peakmerger", "harvestpeaks", numlim=numlim, omlim=omlim
        )
        if self.quiet == "No":
            npks = len(self.parent.guicommander.getdata("peakmerger", "allpeaks"))
            showinfo(
                "Harvested peaks",
                "You have a total of %d peaks," % (npks) + "no peaks have been merged",
            )
    ...
    def mergepeaks(self):
        ...
        if self.quiet == "No":
            showinfo(
                ...
                "You have a total of " + str(nmerged) + " after merging",
            )
        return
