...
EXPONENTIALS = [
    h + str(v) + t
    for v in ijs
    for h, t in [("eps", ""), ("eps", "_s"), ("sig", ""), ("sig", "_s")]
]

#                              'e' ij1  'e' ij2   '_s'  for triangle ij
EXPONENTIALS += [
    "%s%d%s%d%s" % (h, ijs[i], h, ijs[j], t)
    for i in range(6)
    for j in range(i, 6)
...
class columnfile(object):
    ...
    def set_bigarray(self, ar):
        ...
        assert len(ar) == len(self.titles), "Wrong length %d to set bigarray" % (
            len(ar)
        ) + " ".join(self.titles)
        nrows = len(ar[0])
        for col in ar:
    ...
    def updateGeometry(self, pars=None):
        ...
        tth, eta = transform.compute_tth_eta_from_xyz(peaks_xyz, om, **pars.parameters)
...
try:
    ...
    def colfile_to_hdf(
        colfile, hdffile, name=None, compression=None, compression_opts=None
    ):
        """
    ...
    def colfile_from_hdf(hdffile, name=None, obj=None):
        ...
        if name is not None:
            ...
            groups = [
                g
                for g in groups
                if "ImageD11_type" in h[g].attrs
                and h[g].attrs["ImageD11_type"] in ("peaks", b"peaks")
            ]
            assert len(groups) == 1, "Your hdf file has many groups. Which one??" + str(
                groups
            )
            g = h[groups[0]]
            name = groups[0]
...
def colfile2db(colfilename, dbname):
    ...
    ins = "insert into peaks values (" + ",".join(["?"] * colf.ncols) + ") ;"
    # insert the data
