...
from ImageD11 import transform, parameters, ImageD11options
...
def monoclinic_a(cp):
    a, b, c, al, be, ga = cp
    return [a, b, c, al, 90.0, 90.0]


def monoclinic_b(cp):
    a, b, c, al, be, ga = cp
    return [a, b, c, 90.0, be, 90.0]


def monoclinic_c(cp):
    a, b, c, al, be, ga = cp
    return [a, b, c, 90.0, 90.0, ga]
...
def tetragonal(cp):
    """a=b, c, 90,90,90"""
    a, b, c, al, be, ga = cp
    return [(a + b) / 2.0, (a + b) / 2.0, c, 90.0, 90.0, 90.0]
...
def trigonalH(cp):
    ...
    a, b, c, al, be, ga = cp
    anew = (a + b) / 2.0
    return [anew, anew, c, 90.0, 90.0, 120.0]
...
def cubic(cp):
    ...
    anew = (cp[0] + cp[1] + cp[2]) / 3.0
    return [anew, anew, anew, 90.0, 90.0, 90.0]
...
def get_options(parser):
    ...
    parser.add_argument(
        ...
        default="triclinic",
        choices=lattices,
        help="Lattice symmetry for choosing orientation from " + "|".join(lattices),
    )
    parser.add_argument(
        "-t",
        "--tol",
...
class refinegrains:
    ...
    def savegrains(self, filename, sort_npks=True):
        ...
        for g, k in gl:
            ...
            assert (
                len(list(self.scandata.keys())) == 1
            ), "Sorry - need to fix for multi data"
    ...
    def loadfiltered(self, filename):
        ...
        if "drlv2" not in col.titles:
            col.addcolumn(numpy.ones(col.nrows, float), "drlv2")
        if "labels" not in col.titles:
            col.addcolumn(numpy.ones(col.nrows, float) - 2, "labels")
        if "sc" not in col.titles:
            assert "xc" in col.titles
            col.addcolumn(col.xc.copy(), "sc")
        if "fc" not in col.titles:
    ...
    def generate_grains(self):
        ...
        for grainname in self.grainnames:
            for scanname in self.scannames:
                try:
                    _ = self.grains[(grainname, scanname)]
                except KeyError:
    ...
    def compute_gv(self, thisgrain, update_columns=False):
        ...
        if self.OMEGA_FLOAT:
            ...
            _ = cImageD11.score_and_refine(mat, gvT, self.tolerance)
        ...
        if update_columns:
            ...
            numpy.put(self.scandata[name].tth_per_grain, thisgrain.ind, self.tth)
            numpy.put(self.scandata[name].eta_per_grain, thisgrain.ind, self.eta)
            if self.OMEGA_FLOAT:
                numpy.put(
                    self.scandata[name].omegacalc_per_grain, thisgrain.ind, omega_calc
                )
        ...
        for key in self.grains_to_refine:
            ...
            if self.recompute_xlylzl:
                g.peaks_xyz = transform.compute_xyz_lab(
                    [g.sc, g.fc], **self.parameterobj.parameters
                ).T
    ...
    def assignlabels(self, quiet=False):
        ...
        for s in self.scannames:
            ...
            for ig, g in enumerate(self.grainnames):
                ...
                cImageD11.score_and_assign(
                    gr.ubi, gv, self.tolerance, drlv2, int_tmp, int(g)
                )
            if not quiet:
            ...
            for g in self.grainnames:
                ...
                tth, eta = transform.compute_tth_eta_from_xyz(
                    gr.peaks_xyz.T, omega=gr.om * sign, **self.parameterobj.parameters
                )
            ...
            for g in self.grainnames:
                ...
                gr.intensity_info = compute_total_intensity(
                    self.scandata[s], gr.ind, self.intensity_tth_range, quiet=quiet
                )
...
def compute_lp_factor(colfile, **kwds):
    ...
    if "tth_per_grain" in colfile.titles and "eta_per_grain" in colfile.titles:
...
def compute_total_intensity(colfile, indices, tth_range, ntrim=2, quiet=False):
    ...
    if tth_range is not None:
        ...
        intensities = numpy.compress(
            (tth_vals > min(tth_range)) & (tth_vals < max(tth_range)), intensities
        )
        if not quiet:
    ...
    try:
        ret = (
            "sum_of_all = %f , middle %d from %f to %f in tth: median = %f , min = %f , max = %f , mean = %f , std = %f , n = %d"
            % (
                sigma_i,
                len(intensities),
                min(tth_range),
