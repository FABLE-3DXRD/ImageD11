...
def find_lattice(
    ...
    for i, j, k in iter3d(n_try):
        ...
        try:
            if gen_dir == "row":
                if dot(vecs[i], vecs[i]) < min_vec2:
                    continue
                if dot(vecs[j], vecs[j]) < min_vec2:
                    continue
                if dot(vecs[k], vecs[k]) < min_vec2:
                    continue
                print(i, j, k, end=" ")
                l = lattice(
                    vecs[i], vecs[j], vecs[k], direction=gen_dir, min_vec2=min_vec2
                )
            elif gen_dir == "col":
                try:
                    if dot(vecs[:, i], vecs[:, i]) < min_vec2:
...
def cosangle_vec(ubi, v):
    ...
    real = dot(ubi.T, v)
    reci = dot(inv(ubi), v)
    return dot(real, reci) / sqrt(dot(real, real) * dot(reci, reci))
