...
class rsv(object):
    ...
    def normalise(self, savespace=True):
        ...
        for i in range(self.NORMED.shape[0]):
            ...
            numpy.divide(self.SIG[i], self.MON[i], self.NORMED[i])  # divide by mon + 1
    ...
    def slice(self, plane, num):
        ...
        if plane not in self.plnames:
            raise Exception("Plane should be one of %s" % (str(self.plnames)))
...
def readvol(filename, savespace=False):
    ...
    if "signal" not in list(volfile.keys()):  # listnames():
