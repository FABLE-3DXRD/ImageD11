...
def m_to_string(m):
    ...
    st = []
    for i in range(3):
        for v, s in zip([[1, 0, 0], [0, 1, 0], [0, 0, 1]], "xyz"):
            c = np.dot(v, m.T[i])
            if abs(c) > 0:
...
improper_centrosymmetrical_point_groups = (
    """-1 2/m -3 4/m 6/m mmm -3m 4/mmm 6/mmm m-3 m3m""".split()
)
...
def test():
    ...
    assert np.allclose(
        m_from_string("-y,x,z"), np.array([[0, 1, 0], [-1, 0, 0], [0, 0, 1]])
    )
    assert np.allclose(
        m_from_string("-y,y-x,z"), np.array([[0, -1, 0], [-1, 1, 0], [0, 0, 1]])
    )
    print("testing1")
    ...
    assert len(trigonal().group) == 6, "not 7 ops found for trigonal !" + str(
        trigonal().group
    )
    ...
    assert np.allclose(
        find_uniq_u(np.array([[0, 1, 0], [-1, 0, 0], [0, 0, 1]]), cubic()),
        np.identity(3),
    ), "Should easily get this unique choice"
...
if __name__ == "__main__":
    ...
    u = np.array(
        [
            [0.71850787, 0.69517833, 0.02176059],
