...
class SegmenterGui:
    ...
    def __init__(
        ...
        cut_slider = widgets.IntSlider(
            value=cut, min=0, max=200, step=1, description="Cut:"
        )
        pixels_in_spot_slider = widgets.IntSlider(
            value=pixels_in_spot,
            min=0,
            max=20,
            step=1,
        ...
        self.widget = widgets.interactive(
            self.update_image,
            cut=cut_slider,
            pixels_in_spot=pixels_in_spot_slider,
            howmany=howmany_slider,
        )
        display(self.widget)

    def chooseframe(self, counter):
        self.scan, self.idx, _ = chooseframe(
            self.dset, self.scan, self.idx, counter, fetch_raw_image=False
        )
    ...
    def segment_frame(self):
        ...
        with h5py.File(ds.masterfile, "r") as hin:
            ...
            if opts.bg_is_stack:
                bgdataset = opts.bg_dataset or frms.name
                with h5py.File(opts.bgname, "r") as bg_h5:
                    if bgdataset not in bg_h5:
                        raise KeyError(
                            "Missing background dataset %s in %s"
                            % (bgdataset, opts.bgname)
                        )
        ...
        if spf is None:
            print("Warning: no pixels found", self.scan, self.idx)
    ...
    def display(self):
        self.fig, self.axs = plt.subplots(
            1, 2, sharex=True, sharey=True, figsize=(10, 5), constrained_layout=True
        )
        self.im1 = self.axs[0].imshow(
            self.raw_image,
            cmap="viridis",
            norm="log",
            vmin=0.5,
        ...
        self.fig.suptitle(
            "%d peaks found with cut=%d, pixels_in_spot=%d, howmany=%d "
            % (
                self.nblobs,
                self.options["cut"],
                self.options["pixels_in_spot"],
                self.options["howmany"],
            )
        )
    ...
    def update_image(self, cut, pixels_in_spot, howmany):
        ...
        self.fig.suptitle(
            "%d peaks found with cut=%d, pixels_in_spot=%d, howmany=%d "
            % (self.nblobs, cut, pixels_in_spot, howmany_exp)
        )
        self.fig.canvas.draw()

    def getopts(self):
        opts = {
            name: self.options[name] for name in ("cut", "pixels_in_spot", "howmany")
        }
...
class FrelonSegmenterGui:
    ...
    def __init__(
        ...
        **options,
        ...
        self.idx = frame
        self.chooseframe(counter)
        thresh_slider = widgets.IntSlider(
        ...
        smsig_slider = widgets.FloatSlider(
            value=options["smoothsigma"],
            min=0.0,
            max=3.0,
            step=0.05,
            description="Smoothsigma:",
        )
        bgc_slider = widgets.FloatSlider(
            value=options["bgc"], min=0.0, max=1.0, step=0.05, description="bgc:"
        )
        minpx_slider = widgets.IntSlider(
            value=options["minpx"], min=0, max=50, step=1, description="minpx:"
        )
        mofft_slider = widgets.IntSlider(
            value=options["m_offset_thresh"],
            min=0,
            max=200,
            step=1,
            description="m_offset_thresh:",
        )
        mratt_slider = widgets.IntSlider(
            value=options["m_ratio_thresh"],
            min=0,
            max=2000,
            step=1,
    ...
    def chooseframe(self, counter):
        self.scan, self.idx, self.raw_image = chooseframe(
            self.dset, self.scan, self.idx, counter, fetch_raw_image=True
        )
    ...
    def display(self):
        self.fig, self.axs = plt.subplots(
            1, 3, sharex=True, sharey=True, figsize=(10, 3.5), layout="constrained"
        )
    ...
    def update_image(
        self, threshold, smoothsigma, bgc, minpx, m_offset_thresh, m_ratio_thresh
    ):
