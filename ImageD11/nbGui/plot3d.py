...
class Plot3d:
    def __init__(self, xyz=demodata(), w=386, h=256, rx=0.0, ry=0.0, rz=0.0, npx=1):
        self.rgba = np.empty((h, w, 4), "B")
        self.xyz = xyz
        self.npx = npx
        self.ipyimg = ipywidgets.Image()
        self.wrx = ipywidgets.FloatSlider(
            value=rx,
            min=-360,
            max=360.0,
            step=1,
        ...
        self.wry = ipywidgets.FloatSlider(
            value=ry,
            min=-360,
            max=360.0,
            step=1,
        ...
        self.wrz = ipywidgets.FloatSlider(
            value=rz,
            min=-360,
            max=360.0,
            step=1,
    ...
    def redraw(self, change):
        u = scipy.spatial.transform.Rotation.from_euler(
            "XYZ", (self.wrx.value, self.wry.value, self.wrz.value), degrees=True
        ).as_matrix()
