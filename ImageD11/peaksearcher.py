...
def peaksearch(filename, data_object, corrector, thresholds, labims):
    """
...
def peaksearch_driver(options, args):
    ...
    if scan is None:
        if options.interlaced:
            f0 = [
                "%s0_%04d%s" % (options.stem, i, options.format)
                for i in range(options.first, options.last + 1)
            ]
            f1 = [
                "%s1_%04d%s" % (options.stem, i, options.format)
                for i in range(options.first, options.last + 1)
            ]
    ...
    for t in thresholds_list:
        ...
        li_objs[t] = labelimage(
            shape=s, fileout=mergefile, spatial=corrfunc, sptfile=spotfile
        )
            ...
            for data_object in file_series_object:
                ...
                if not hasattr(data_object, "data"):
                    ...
                    if isinstance(data_object[1], IOError):
                        ...
                        traceback.print_exception(
                            data_object[0], data_object[1], data_object[2]
                        )
                        sys.exit()
                    continue
                ...
                if not OMEGAOVERRIDE and options.omegamotor != "Omega":
                    data_object.header["Omega"] = float(
                        data_object.header[options.omegamotor]
                    )
                data_object = correct(
                    data_object,
                    darkimage,
                ...
                peaksearch(filein, data_object, corrfunc, thresholds_list, li_objs)
            for t in thresholds_list:
                li_objs[t].finalise()
        ...
        try:
            ...
            class read_only(ImageD11_thread.ImageD11_thread):
                def __init__(
                    ...
                    ImageD11_thread.ImageD11_thread.__init__(self, myname=myname)
                    ...
                    for data_object in self.file_series_obj:
                        ...
                        try:
                            if self.OMEGAOVERRIDE:
                                ...
                                if (
                                    options.omegamotor != "Omega"
                                    and options.omegamotor in data_object.header
                                ):
                                    data_object.header["Omega"] = float(
                                        data_object.header[options.omegamotor]
                                    )
            ...
            class peaksearch_one(ImageD11_thread.ImageD11_thread):
                def __init__(
                    self, q, corrfunc, threshold, li_obj, myname="peaksearch_one"
                ):
            ...
            while nalive > 0:
                try:
                    ...
                    for thr in my_threads:
                        if thr.is_alive():
                            nalive += 1
                    if options.killfile is not None and os.path.exists(
                        options.killfile
                    ):
...
def get_options(parser):
    ...
    parser.add_argument(
        ...
        default=dod,
        type=float,
        help="Constant to subtract from dark to avoid overflows, default=%d" % (dod),
    )
