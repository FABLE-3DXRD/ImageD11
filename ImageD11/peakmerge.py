...
class peak:
    ...
    def combine(self, other):
        ...
        if self.threshold < other.threshold and self.omega == other.omega:
            ...
            if self.threshold > other.threshold and self.omega == other.omega:
                ...
                avg = s / np
                omega = (
                    self.omega * self.np * self.avg + other.omega * other.np * other.avg
                ) / s
                num = (
                    self.num * self.np * self.avg + other.num * other.np * other.avg
                ) / s
                x = (self.x * self.np * self.avg + other.x * other.np * other.avg) / s
                y = (self.y * self.np * self.avg + other.y * other.np * other.avg) / s
                xc = (
                    self.xc * self.np * self.avg + other.xc * other.np * other.avg
                ) / s
                yc = (
                    self.yc * self.np * self.avg + other.yc * other.np * other.avg
                ) / s
                sigx = (
                    self.sigx * self.np * self.avg + other.sigx * other.np * other.avg
                ) / s
                sigy = (
                    self.sigy * self.np * self.avg + other.sigy * other.np * other.avg
                ) / s
                covxy = (
                    self.covxy * self.np * self.avg + other.covxy * other.np * other.avg
                ) / s
                self.forgotten = True
                other.forgotten = True
                # Make a new line
    ...
    def __eq__(self, other):
        ...
        try:
            #           print "using __eq__"
            if (
                abs(self.xc - other.xc) < self.TOLERANCE
                and abs(self.yc - other.yc) < self.TOLERANCE
            ):
                return True
            else:
                return False
...
class peakmerger:
    ...
    def readpeaks(self, filename, startom=0.0, omstep=1.0):
        """
    ...
    def harvestpeaks(self, numlim=None, omlim=None, thresholds=None):
        ...
        if numlim is None:
            ...
            def numinrangex(x):
                return x > min(numlim) and x < max(numlim)
        ...
        if omlim is None:
            ...
            def ominrange(x):
                return x > min(omlim) and x < max(omlim)

        if thresholds is None:

            def thresholdsinrange(x):
                True

        else:

            def thresholdsinrange(x):
                x > min(thresholds) and x < max(thresholds)
            ...
            if numinrange(image.imagenumber) and ominrange(om):
                ...
                while line.find("# File") < 0 and i < maxi:
                    ...
                    if (
                        line[0] != "#"
                        and len(line) > 10
                        and thresholdsinrange(threshold)
                    ):
        ...
        logging.info("Time to read into lists %f" % (time.time() - start_harvest))
        start_harvest = time.time()
        ...
        logging.info(
            "Time to DSU sort: %f %d" % (time.time() - start_harvest, len(peaks))
        )
    ...
    def mergepeaks(self):
        ...
        while i < npeaks:
            ...
            if (
                self.allpeaks[i] == merge1[-1]
                and abs(self.allpeaks[i].omega - merge1[-1].omega) < merge1[-1].omegatol
            ):
        ...
        npeaks = len(peaks)
        logging.info(
            "After merging equivalent omegas npeaks = %d time = %f"
        ...
        logging.info(
            "Number of different omegas = %d time = %f"
        ...
        while i < nomega - 2:
            ...
            debug += " %-5d peaks on next, %-5d in buffer" % (nextlast - last, ncarry)
            logging.debug(debug)
            # sys.stdout.flush()
            for peak1 in active:
                m = 0
        ...
        logging.info("You have a total of " + str(len(self.merged)) + " after merging")
    ...
    def filter(self):
        ...
        for p in self.merged:
            biglist.append(
                [p.xc, p.yc, p.omega, p.np, p.avg, p.x, p.y, p.sigx, p.sigy, p.covxy]
            )
