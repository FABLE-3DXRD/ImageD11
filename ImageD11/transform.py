...
def compute_sinsqth_from_xyz(xyz):
    ...
    if you need grain translations then use:
             func(peaks_xyz - compute_grain_origins(...) )
...
def compute_tth_histo(tth, no_bins=100, weight=False, weights=None, **kwds):
    ...
    New feature: weight by something (peak intensity for instance), send
    true for weight and weights values
    ...
    *and*
    ...
    # Tried to use numpy's digitize but failed. Edges are treated differently between
    # np.histogram and np.digitize (both are inclusive in np.histogram)
