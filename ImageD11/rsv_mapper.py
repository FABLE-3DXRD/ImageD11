...
class rsv_mapper(object):
    ...
    def find_vol(self, border, omegarange):
        ...
        tth, eta = transform.compute_tth_eta(peaks, **self.pars.get_parameters())
    ...
    def make_k_vecs(self):
        ...
        tth, eta = transform.compute_tth_eta(peaks, **self.pars.get_parameters())
        self.k = transform.compute_k_vectors(tth, eta, self.pars.get("wavelength"))
    ...
    def add_image(self, om, data):
        ...
        numpy.add(
            ind,
            NR[2] * numpy.floor(hkls[1] + 0.5 - self.bounds[1][0]).astype(numpy.intp),
            ind,
        )
        numpy.add(
            ind, numpy.floor(hkls[2] + 0.5 - self.bounds[2][0]).astype(numpy.intp), ind
        )
        #
        #
        #
...
def main():
    ...
    try:
        ...
        print("%.5f %.5f %.5f %.4f %.4f %.4f" % indexing.ubitocellpars(ubi))
    except:
