...
from ImageD11.cImageD11 import (
    s_1,
    s_I,
    s_I2,
    bb_mn_f,
...
class labelimage:
    ...
    def labelpeaks(self, data, threshold):
        ...
        self.npk = cImageD11.connectedpixels(
            data.astype(np.float32), self.blim, threshold, self.verbose
        )
    ...
    def measurepeaks(self, data, omega, blim=None):
        if blim is not None:
            ...
            self.blim = blim
        if self.npk > 0:
            self.res = cImageD11.blobproperties(
                data.astype(np.float32), self.blim, self.npk, omega=omega
            )
        else:
            # What to do?
            self.res = None
    ...
    def mergelast(self):
        ...
        if self.npk > 0 and self.lastnp > 0:
            ...
            #
            self.npk = cImageD11.bloboverlaps(
                self.lastbl,
                self.lastnp,
                self.lastres,
                self.blim,
    ...
    def output2dpeaks(self, file_obj):
        ...
        file_obj.write(
            "# Number_of_pixels Average_counts    s   f     sc   fc      sig_s  sig_f  cov_sf  IMax_int\n"
        )
