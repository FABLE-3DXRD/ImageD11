...
data = (numpy.random.random( 2048*2048 )*10000).astype(numpy.uint16)

lib = ctypes.CDLL( "imsum.so" )
lib.imsum_sse2_0.argtypes = [ ctypes.POINTER( ctypes.c_uint16 ), ctypes.c_int ]
lib.imsum_sse2_0.restype  = ctypes.c_int64
lib.imsum_sse2_1.argtypes = [ ctypes.POINTER( ctypes.c_uint16 ), ctypes.c_int ]
lib.imsum_sse2_1.restype  = ctypes.c_int64
lib.imsum_avx2_0.argtypes = [ ctypes.POINTER( ctypes.c_uint16 ), ctypes.c_int ]
lib.imsum_avx2_0.restype  = ctypes.c_int64
lib.imsum_avx2_1.argtypes = [ ctypes.POINTER( ctypes.c_uint16 ), ctypes.c_int ]
lib.imsum_avx2_1.restype  = ctypes.c_int64
lib.imsum_c.argtypes = [ ctypes.POINTER( ctypes.c_uint16 ), ctypes.c_int ]
...
N=100
t0=time.time()
vals = [ data.sum() for i in range(N)]
t1=time.time()
avg = (t1-t0)/N
print("numpy",avg)
for cfunc in cfuns:
