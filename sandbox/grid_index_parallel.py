...
def grain_merge( oldgrain, newgrain ):
    """ Average the translation position and increment nfound """
    t = oldgrain.translation * oldgrain.nfound
    t += newgrain.translation
    oldgrain.nfound += 1
    oldgrain.translation = t / oldgrain.nfound
        ...
        for i,gnew in enumerate(grains):
            ...
            for gold in self.uniqgrains:
                ...
                if angle < self.tar:
                    ...
                    print( "           matched %d %.4f %.4f"%(i,np.degrees(angle),np.sqrt(dt2)), end='' )
                    print( " %.1f %.1f %.1f "%tuple(gold.translation), end='' )
                    grain_merge( gold, gnew )
                    print( "  %.1f %.1f %.1f"%tuple(gold.translation) )
                    newgrain = False
                    break
            if newgrain:
