...
def refine_point_axel(xi0, yi0, ubis, params, peak_dict, lattice):
    ...
    for i,g in enumerate(grains):
        ...
        # recompute the peaks this grains likes with merged data
...
def count_unique_peaks(hkl, etasign, dtyi):
    uniques = np.empty((5, etasign.shape[0]), dtype=etasign.dtype)
    inverses = np.empty((etasign.shape[0]), dtype=np.int64)
    unique_ptr = 0
    # iterate through the input arrays
    for idx in np.arange(etasign.shape[0], dtype=np.int64):
        h, k, l = hkl[:,idx]
        this_etasign = etasign[idx]
        this_dtyi = dtyi[idx]
        
        # iterate through the unique array
        seen_array = False
        for idx_unique in np.arange(uniques.shape[0], dtype=np.int64):
            ...
            if (h == h_uniq) & (k == k_uniq) & (l == l_uniq) & (this_etasign == etasign_uniq) & (this_dtyi == dtyi_uniq):
                # saw it already!:
                inverses[idx] = idx_unique
                seen_array = True
        if not seen_array:
            # got to the end of uniques and didn't see it
            # uniques[:, unique_ptr] = [h, k, l, this_etasign, this_dtyi]
            uniques[0, unique_ptr] = h
            uniques[1, unique_ptr] = k
            uniques[2, unique_ptr] = l
            uniques[3, unique_ptr] = this_etasign
            uniques[4, unique_ptr] = this_dtyi
            
            inverses[idx] = unique_ptr
            unique_ptr += 1
...
def mine(refine_points, all_pbpmap_ubis, ri_col, rj_col, sx_grid, sy_grid,  # refinement stuff
    ...
    for refine_idx in numba.prange(len(refine_points)):
        ...
        # compute g-vectors at this voxel
        this_distance = distance - xpos[local_mask]
        
        tth_gve, eta_gve = compute_tth_eta(sc[local_mask], fc[local_mask],
                                    y_center=y_center,
                                    y_size=y_size,
                                    tilt_y=tilt_y,
                                    z_center=z_center,
        ...
        gve = compute_g_vectors(tth_gve, eta_gve,
                                omega[local_mask],
                                wavelength,
                                wedge=wedge,
                                chi=chi)
        ...
        for ubi_idx in np.arange(ubis_here.shape[2]):
            ...
            j, labels, drlv2 = score_and_assign(ubi, gve, tol, ubi_idx)
            ...
            print(grain_npks, 'unmerged peaks assigned before merging')
            # compute hkl floats
            hkl_double = np.dot(ubi, gve[:, grain_peak_mask])
            # hkl ints
            ...
            # print('did merge')
            # re-compute peak mask with merged peaks
            local_mask_of_grain, ydist = get_voxel_mask(y0, xi0, yi0, merged_sinomega, merged_cosomega, merged_dty, ystep)
            
            # compute g-vectors at this voxel again with merged peaks
            this_distance = distance - merged_xpos_refined[local_mask_of_grain]

            tth_gve_grain, eta_gve_grain = compute_tth_eta(merged_sc[local_mask_of_grain], merged_fc[local_mask_of_grain],
                                        y_center=y_center,
                                        y_size=y_size,
                                        tilt_y=tilt_y,
                                        z_center=z_center,
            ...
            gve_grain = compute_g_vectors(tth_gve_grain, eta_gve_grain,
                                    merged_omega[local_mask_of_grain],
                                    wavelength,
                                    wedge=wedge,
                                    chi=chi)
            
            # print('reassigning')
            # reassign merged g-vectors
            j, labels, drlv2 = score_and_assign(ubi, gve_grain, tol, ubi_idx)
            grain_peak_mask = labels == ubi_idx
            grain_npks = np.sum(grain_peak_mask)
            
            # re-mask g-vectors to those assigned
            grain_gve = gve_grain[:, grain_peak_mask]
            hkl_double = np.dot(ubi, grain_gve)
            hkl = np.round(hkl_double)
